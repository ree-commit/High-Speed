...
TcpSocketBase::TcpSocketBase (const TcpSocketBase& sock)
  : TcpSocket (sock),
    ...
    m_resetRtoUponPartialAck (sock.m_resetRtoUponPartialAck),
...
TcpSocketBase::ReceivedAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  TcpTxItem item;
  if ((ackNumber > oldHeadSequence) && (ackNumber < m_recover) && (m_tcb->m_congState == TcpSocketState::CA_RECOVERY) && (m_recoveryOps->GetName () == "LinuxPrrRecovery"))
    {
      for (uint32_t i = 0; i < segAcked; i++)
        {
          m_txBuffer->GetPacketInfo (ackNumber - (i * m_tcb->m_segmentSize), &item);
          if (item.m_retrans)
            {
      ...
      if (ackNumber < m_recover && m_tcb->m_congState == TcpSocketState::CA_RECOVERY)
        {
          ...
          TcpTxItem item;
          m_txBuffer->GetPacketInfo (ackNumber + m_tcb->m_segmentSize, &item);

          // If the packet is already retransmitted do not retransmit it
          if (AvailableWindow () > 0 && !item.m_retrans)
            {
          ...
          NewAck (ackNumber ,m_resetRtoUponPartialAck);
          //if (m_isFirstPartialAck)
	  if(m_resetRtoUponPartialAck && m_tcb->m_congState == TcpSocketState::CA_LOSS)		
            {
              ...
              m_resetRtoUponPartialAck = false;
            }
          else
            {
              NS_LOG_DEBUG ("Partial ACK of " << ackNumber <<
